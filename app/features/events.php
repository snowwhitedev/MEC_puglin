...
class MEC_feature_events extends MEC_base
{
    ...
    ?>
        <div class="mec-add-booking-tabs-wrap">
            <div class="mec-add-booking-tabs-left">
                <a class="mec-add-booking-tabs-link mec-tab-active" data-href="mec_meta_box_booking_options_form_1" href="#"><?php echo esc_html('Total Booking Limits' ,'mec'); ?></a>
                <a class="mec-add-booking-tabs-link" data-href="mec_meta_box_booking_options_form_2" href="#"><?php echo esc_html('Total User Booking Limits' ,'mec'); ?></a>
                <a class="mec-add-booking-tabs-link" data-href="mec-tickets" href="#"><?php echo esc_html('Tickets' ,'mec'); ?></a>
                <?php if(isset($this->settings['taxes_fees_status']) and $this->settings['taxes_fees_status']) : ?>
                <a class="mec-add-booking-tabs-link" data-href="mec-fees" href="#"><?php echo esc_html('Fees' ,'mec'); ?></a>
                <?php endif; ?>
                <?php if(isset($this->settings['ticket_variations_status']) and $this->settings['ticket_variations_status']) : ?>
                <a class="mec-add-booking-tabs-link" data-href="mec-ticket-variations" href="#"><?php echo esc_html('Ticket Variations / Options' ,'mec'); ?></a>
                <?php endif; ?>
                <a class="mec-add-booking-tabs-link" data-href="mec-reg-fields" href="#"><?php echo esc_html('Booking Form' ,'mec'); ?></a>
                <?php if ( isset( $gateway_settings['op_status'] ) && $gateway_settings['op_status'] == 1 ) : ?>
                <a class="mec-add-booking-tabs-link" data-href="mec_meta_box_op_form" href="#"><?php echo esc_html('Organizer Payment' ,'mec'); ?></a>
                <?php endif; ?>
    ...
    public function action_links($actions, $post)
    {
        ...
        $booking_status = (isset($this->settings['booking_status']) and $this->settings['booking_status']) ? true : false;
        if ( $booking_status ){
            $actions['mec-attendees'] = '<a class="mec-event-attendees" href="#" data-id="'.$post->ID.'">'.__('Attendees', 'mec').'</a>
            ...
            {
                $actions['mec-attendees'] .= '<a href="#" data-value="'.$date->dstart.'" onclick="mec_event_attendees('.$post->ID.', \''.$date->dstart.'\');" '.($occurrence == $date->dstart ? 'class="selected-day"' : '').'>'.(($date->dstart != $date->dend) ? sprintf(__('%s to %s', 'mec'), $date->dstart, $date->dend) : $date->dstart).'</a>';
            }
            ...
            if(count($attendees))
            {
                ...
                foreach($attendees as $attendee)
                {
                    ...
                    $actions['mec-attendees'] .= '<div class="w-col-xs-3">'.((isset($attendee['id']) and isset($tickets[$attendee['id']]['name'])) ? $tickets[$attendee['id']]['name'] : __('Unknown', 'mec')).'</div>';
            ...
            {
                $actions['mec-attendees'] .= '<p>'.__("No Attendees Found!", 'mec').'</p>';
            }
    ...
    public function attendees()
    {
        ...
        if(count($attendees))
        {
            ...
            foreach($attendees as $attendee)
            {
                ...
                $html .= '<div class="w-col-xs-3">'.((isset($attendee['id']) and isset($tickets[$attendee['id']]['name'])) ? $tickets[$attendee['id']]['name'] : __('Unknown', 'mec')).'</div>';
