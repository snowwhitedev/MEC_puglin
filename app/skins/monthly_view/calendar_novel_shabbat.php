        ...
        if (isset($events[$today]) and count($events[$today])) {
            ...
            foreach ($events[$today] as $event) {
                ...
                foreach ($labels as $id => $label) {
                    if ($label['name'] == SHABBAT) {
                        $shabbat_found = true;
                        if (!trim($shabbat_permalink)) {
                            $shabbat_permalink = $event->data->permalink;
                        }
                        break;
                    }
                }
                if ($shabbat_found) break;
            }
            if ($shabbat_found & $today >= date('Y-m-d')) {
                echo '<dt onclick="mec_shabbat_select(' . "'" . $today . "'"  . ')" class="mec-calendar-day mec-shabbat-day' . $selected_day . '" id="day-' . $today . '" data-mec-cell="' . $day_id . '" data-day="' . $list_day . '" data-month="' . date('Ym', $time) . '"><div class="mec-calendar-novel-selected-day' . $selected_day_date . '">' . $list_day . '</div>';
            } else {
            ...
            foreach ($events[$today] as $event) {
                ...
                if ($shabbat_found && !$shabbat_label) {
                    continue;
                }
                $speakers = '""';
...
<script type="text/javascript">
    var selected_shabbats = [];

    function mec_shabbat_select(day_id) {
        if(jQuery("#shabbat_date_selectable").val() != "1"){
            alert("You can not select shabbat days here.");
            return;
        }
        var obj_id = "#day-" + day_id;

        if (jQuery(obj_id).hasClass("mec-shabbat-selected")) {
            jQuery(obj_id).removeClass("mec-shabbat-selected");
            pos = selected_shabbats.indexOf(day_id)
            if (pos >= 0) {
                selected_shabbats.splice(pos, 1);
            }
        } else {
            jQuery(obj_id).addClass("mec-shabbat-selected");
            selected_shabbats.push(day_id);
        }

        var selected_shabbat_dates = selected_shabbats.join(",");

        jQuery("#mec_shabbat_customer_date").val(selected_shabbat_dates);
    }
</script>