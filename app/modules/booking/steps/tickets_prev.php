...
<form id="mec_book_form<?php echo $uniqueid; ?>">
    ...
    <div class="mec-book-first">
        <?php if($shabbat):?>
            <input type="hidden" name="book[customer_date]" value="<?php echo $customer_date;?>" id="mec_shabbat_customer_date" readonly/>
            <input type="hidden" name="book[date]" value="<?php echo $occurrence;?>"/>
        <?php else:?>
       ...
       <?php foreach ($tickets as $ticket_id => $ticket) :
            ...
            <div class="mec-event-ticket mec-event-ticket<?php echo $ticket_limit; ?>" id="mec_event_ticket<?php echo $ticket_id; ?>">
                <div class="mec-ticket-available-spots <?php echo ($ticket_limit == '0' ? 'mec-util-hidden' : ''); ?>">
                    ...
                    <div>
                        <input type="number" class="mec-book-ticket-limit" name="book[tickets][<?php echo $ticket_id; ?>]"
                            placeholder="<?php esc_attr_e('Count', 'mec'); ?>"
                            value="<?php echo isset($_REQUEST['prev_book']['tickets'])? $_REQUEST['prev_book']['tickets'][$ticket_id]:$default_ticket_number; ?>"
                            min="0" max="<?php echo ($ticket_limit != '-1' ? $ticket_limit : ''); ?>"
                            onchange="mec_check_tickets_availability<?php echo $uniqueid; ?>(<?php echo $ticket_id; ?>, this.value);" />
                    </div>
                    <!--<span class="mec-event-ticket-available">
                        <?php //echo sprintf(__('Available %s: <span>%s</span>', 'mec'), __('Tickets', 'mec'), ($ticket['unlimited'] ? __('Unlimited', 'mec') : ($ticket_limit != '-1' ? $ticket_limit : __('Unlimited', 'mec')))); ?>
                    </span>-->
                </div>
    ...
    <?php if ($this->main->get_recaptcha_status('booking')) : ?>
        <div class="mec-google-recaptcha">
            <div id="g-recaptcha" class="g-recaptcha" data-sitekey="<?php echo $settings['google_recaptcha_sitekey']; ?>"></div>
        </div>
    <?php endif; ?>
    ...
    <input type="hidden" name="form_direction" value="next"/>
...
<!---->
<input type="hidden" id="shabbat_date_selectable" value="1"/>