...
<form id="mec_book_form<?php echo $uniqueid; ?>">
    ...
    <div class="mec-book-first">
        <input type="hidden" name="book[customer_date]" id="mec_shabbat_customer_date" value="<?php echo $customer_date;?>" readonly/>
        ...
        <?php foreach ($tickets as $ticket_id => $ticket) :
            ...
            <div class="mec-event-ticket mec-event-ticket<?php echo $ticket_limit; ?>" id="mec_event_ticket<?php echo $ticket_id; ?>">
                <div class="mec-ticket-available-spots <?php echo ($ticket_limit == '0' ? 'mec-util-hidden' : ''); ?>">
                    <span class="mec-event-ticket-name">
                        <?php echo (isset($ticket['name']) ? $ticket['name'] : ''); ?>
                    </span>
                    <span class="mec-event-ticket-price">
                        <?php echo (isset($ticket['price_label']) ? $book->get_ticket_price_label($ticket, current_time('Y-m-d')) : ''); ?>
                    </span>
                    <?php if (isset($ticket['description']) and trim($ticket['description'])) : ?>
                        <p class="mec-event-ticket-description"><?php echo $ticket['description']; ?></p>
                    <?php endif; ?>
                    <div>
    ...
    <?php if ($this->get_recaptcha_status('booking')) : ?>
    ...
    <input type="hidden" name="shabbat" id="shabbat" value="1">
