...
<form id="mec_book_form<?php echo $uniqueid; ?>" novalidate="novalidate" enctype="multipart/form-data" method="post">
    <h4><?php echo apply_filters('mec-attendees-title', __('Attendees Form', 'mec')) ?></h4>
    <ul class="mec-book-tickets-container">
            ...
            for ($i = 1; $i <= $count; $i++) : $j++; ?>
                <li class="mec-book-ticket-container <?php echo (($j > 1 and $first_for_all) ? 'mec-util-hidden' : ''); ?>">
                    ...
                    ?>
                            ...
                            <div class="mec-book-reg-field-<?php echo $reg_field['type']; ?>
                                <?php echo ((isset($reg_field['mandatory']) and $reg_field['mandatory']) ? 'mec-reg-mandatory' : ''); ?>
                                <?php
                                ...
                                <?php
                                if (
                                    isset($reg_field['label']) and $reg_field['type'] != 'agreement'
                                    &&  $reg_field['type'] != 'name' && $reg_field['type'] != 'mec_email'
                                ) :
                                ?>
                                    <label for="mec_book_reg_field_reg<?php echo $j . '_' . $reg_field_id; ?>">
                                        <?php _e($reg_field['label'], 'mec'); ?>
                                        <?php
                                        echo ((isset($reg_field['mandatory']) and $reg_field['mandatory']) ? '<span class="wbmec-mandatory">*</span>' : '');
                                        ?>
                                    </label>
                                <?php endif; ?>
                                ...
                                <?php /** Name **/ if ($reg_field['type'] == 'name') : ?>
                                    ...
                                    <label for="mec_book_reg_field_name<?php echo $reg_field_id; ?>">
                                        <?php _e($reg_field['label'], 'mec'); ?>
                                        <span class="wbmec-mandatory">*</span>
                                    </label>
                                    <input id="mec_book_reg_field_name<?php echo $reg_field_id; ?>" type="text" 
                                        name="book[tickets][<?php echo $j; ?>][name]" 
                                        value="<?php echo trim((isset($current_user->user_firstname) ? $current_user->user_firstname : '') . ' ' . (isset($current_user->user_lastname) ? $current_user->user_lastname : '')); ?>" 
                                        placeholder="<?php _e($reg_field['label'], 'mec'); ?>" required />

                                <?php /** MEC Email **/ elseif ($reg_field['type'] == 'mec_email') : ?>
                                    <?php $reg_field['label'] = ($reg_field['label']) ? $reg_field['label'] : 'Email'; ?>
                                    <label for="mec_book_reg_field_email<?php echo $reg_field_id; ?>">
                                        <?php _e($reg_field['label'], 'mec'); ?>
                                        <span class="wbmec-mandatory">*</span>
                                    </label>
                                    <input id="mec_book_reg_field_email<?php echo $reg_field_id; ?>" type="email" 
                                        name="book[tickets][<?php echo $j; ?>][email]" 
                                        value="<?php echo isset($current_user->user_email) ? $current_user->user_email : ''; ?>" 
                                        placeholder="<?php _e('Email', 'mec'); ?>" required />
                                ...
                                <?php /** Textarea **/ elseif ($reg_field['type'] == 'textarea') : ?>
                                    <textarea id="mec_book_reg_field_reg<?php echo $j . '_' . $reg_field_id; ?>" name="book[tickets][<?php echo $j; ?>][reg][<?php echo $reg_field_id; ?>]" placeholder="<?php _e($reg_field['label'], 'mec'); ?>" <?php if (isset($reg_field['mandatory']) and $reg_field['mandatory']) echo 'required'; ?>>
                                    </textarea>
                                ...
                                <?php /** Paragraph **/ elseif ($reg_field['type'] == 'p') : ?>
                                    <?php
                                    $firstCharacter = substr(stripslashes($reg_field['content']), 0, 1);
                                    if ($firstCharacter == '[') : ?>
        ...
        <?php endfor;?>
            <!--for prev button to calendar form-->
            <input type="hidden" name="prev_book[tickets][<?php echo $ticket_id;?>]" value="<?php echo $count;?>">
        <?php endforeach; ?>

        <?php if ($j > 1 and $first_for_all) : ?>
            <li class="mec-first-for-all-wrapper" style="display: none;">
    ...
    <input type="hidden" name="form_direction" id="form_direction" value="next" />
    <input type="hidden" name="shabbat" id="shabbat" value="<?php echo $shabbat;?>">
