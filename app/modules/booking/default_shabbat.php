...
//If shabbat event and no shabbat dates
...
function mec_book_form_submit' . $uniqueid . '()
{
    ...
    var form_direction = jQuery("#form_direction").val();
    if(form_direction == "next" && step == 2)
    {
        var valid = true;

        jQuery("#mec_book_form' . $uniqueid . ' .mec-book-ticket-container .mec-book-reg-field-customer-date.mec-reg-mandatory").filter(":visible").each(function(i)
        {
            var ticket_id = jQuery(this).data("ticket-id");

            if(!jQuery("#mec_book_form' . $uniqueid . ' input[name=\'book[tickets]["+ticket_id+"][customer-date]\']").val())
            {
                valid = false;
        ...
        jQuery("#mec_book_form' . $uniqueid . ' .mec-book-ticket-container .mec-book-reg-field-email.mec-reg-mandatory").filter(":visible").each(function(i)
        {
            var ticket_id = jQuery(this).data("ticket-id");
    ...
    if(form_direction == "next"){
        jQuery("#mec_book_form' . $uniqueid . ' button[type=submit]").addClass("loading");
    } else if (form_direction == "prev"){
        jQuery("#mec_book_form_prev").addClass("loading");
    }
    jQuery("#mec_book_form' . $uniqueid . ' button[type=submit]").attr("disabled" , "true");
    jQuery("#mec_book_form_prev").attr("disabled", "true");
    ...
    if( step != "5"){
        var sendData = new FormData(jQuery("#mec_book_form' . $uniqueid . '")[0]);
    } else if(step == "5"){ //payment form
        var sendData =new FormData(jQuery("#mec_book_form' . $uniqueid . '")[0]);;
    }
    jQuery.ajax({
        type: "POST",
        ...
        success: function(data)
        {
            ...
            if(form_direction == "next"){
                ...
                jQuery("#mec_book_form_prev").removeClass("loading");
            }
            jQuery("#mec_book_form' . $uniqueid . ' button[type=submit]").removeAttr("disabled");
            jQuery("#mec_book_form_prev").removeAttr("disabled");
            if(data.success)
            {
...
<div class="mec-booking" id="mec_booking<?php echo $uniqueid; ?>">
    <?php
    include MEC::import('app.modules.booking.steps.tickets_shabbat', true, true);
    ?>
</div>
