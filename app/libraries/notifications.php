    ...
    public function content($message, $book_id, $attendee = array())
    {
        ...
        if(trim($book_date) and strpos($book_date, ':') !== false)
        {
            ...
            if(isset($ex[0]) and isset($ex[1]) and trim($ex[0]) != trim($ex[1]))
            {
                ...
                $book_date = sprintf(__('%s to %s', 'mec'), date_i18n($date_format, strtotime($ex[0])), date_i18n($date_format, strtotime($ex[1])));
            }
        ...
        if(isset($book_date) and strpos($book_date, __('to', 'mec')) !== false)
        {
            $explode_time = explode(__('to', 'mec'), $book_date);
