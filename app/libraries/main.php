...
class MEC_main extends MEC_base
{
    ...
    /***custome */
    public function get_current_event_color($id){
        return get_post_meta( $id, 'mec_color', true );
    }
    /**
     * @author Webnus <info@webnus.biz>
     * @param int $post_id
                        ...
                        <div class="mec-addons-notification-box-content">
                            <div class="w-box-content">
                                ...
                                <ol>
                                    ...
                                    <li>'.__('<strong>Elementor Single Builder:</strong> Edit single event page using Elementor. Manage the position of all elements in the Single page and in desktops, mobiles and tablets as well.' , 'mec').'</li>
                                    <li>'.__('<strong>Elementor Shortcode Builder:</strong> It enables you to create shortcodes in Elementor Live Editor.', 'mec').'</li>
                                </ol>
    ...
    public function booking_invoice()
    {
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'mec-invoice')
        {
            ...
            $mec_customer_date = get_post_meta($book_id, 'mec_customer_date', true);
             
            if(!$mec_confirmed and (!current_user_can('administrator') and !current_user_can('editor')))
            {
                wp_die(__('Your booking still is not confirmed. You able download it after confirmation!', 'mec'), __('Booking Not Confirmed.', 'mec'));
                exit;
            }
            ...
            $mec_customer_date_ex = explode("," , $mec_customer_date);
            $total_days = count($mec_customer_date_ex);
            $tickets_table = array();
            ...
            if(isset($transaction['tickets']) and is_array($transaction['tickets']) and count($transaction['tickets']))
            {
                foreach($transaction['tickets'] as $attendee){
                    if(trim($customer_name) == "Unknown"){
                        $customer_name = $attendee['name']; 
                    }
                    if(trim($customer_email == "Unknown")){
                        $customer_email = $attendee['email'];
                    }
                    $t_name = (isset($event->tickets[$attendee['id']])) ? $event->tickets[$attendee['id']]['name'] : 'Unknown';
                    if(array_key_exists($t_name, $tickets_table)){
                        $tickets_table[$t_name]["quantity"]++;
                        $tickets_table[$t_name]["total"] = round($tickets_table[$t_name]["quantity"] * $total_days * floatval($tickets_table[$t_name]["price"]), 2);
                    } else {
                        $t_price = (isset($event->tickets[$attendee['id']])) ? $book->get_ticket_price_label($event->tickets[$attendee['id']], $booking_time) : '';
                        
                        $tickets_table[$t_name] = array(
                            "ticket_name" => $t_name,
                            "quantity" => 1,
                            "days"    => $total_days,
                            "price"   => $t_price,
            ...
            //make html for pdf
            $inv_html = "";
            $inv_html .= '<div><table style="border:1px solid #dcd7ca; border-collapse: collapse; text-align:right; margin:30px auto;"><thead><tr>';
            $inv_html .= '<th style="border:1px solid #dcd7ca; padding:5px;">Tickets</th>' . '<th style="border:1px solid #dcd7ca; padding:5px;">Quantity</th>' . '<th style="border:1px solid #dcd7ca; padding:5px;">Days</th>' . '<th style="border:1px solid #dcd7ca; padding:5px;">Price</th>' . '<th style="border:1px solid #dcd7ca; padding:5px;">Total</th>';
            $inv_html .= '</tr ><thead><tbody>';

            $total_booking_price = 0;
            foreach($tickets_table as $t_name => $item){
                $inv_html .= '<tr><td style="border:1px solid #dcd7ca; padding-top:5px;">';
                $inv_html .= $t_name;
                $inv_html .= '</td><td style="border:1px solid #dcd7ca; padding:5px;">';
                $inv_html .= $item['quantity'];
                $inv_html .= '</td><td style="border:1px solid #dcd7ca; padding:5px;">';
                $inv_html .= $item['days'];
                $inv_html .= '</td><td style="border:1px solid #dcd7ca; padding:5px;">';
                $inv_html .= $this->render_price($item['price']);
                $inv_html .= '</td><td style="border:1px solid #dcd7ca; padding:5px;">';
                $inv_html .= $this->render_price($item['total']);
                $inv_html .= '</td>';
                $inv_html .= '</tr>';
                $total_booking_price += $item['total'];
            }
            $inv_html .= '<tr><td style="border:1px solid #dcd7ca;"></td><td style="border:1px solid #dcd7ca;"></td><td></td><td style="border:1px solid #dcd7ca; color:#39c36e; padding:5px;"><strong>Total</strong></td><td style="border:1px solid #dcd7ca;color:#39c36e; padding:5px;"><strong>';
            $inv_html .= $this->render_price($total_booking_price);
            $inv_html .= '</strong></td></tr>';
            $inv_html .= '</tbody></table></div>';

            $payment_html .= '<div><span><strong>'.__("Name: ", "mec").'</strong></span><span>'. $customer_name .'</span></div>';
            $payment_html .= '<div><span><strong>'.__("Email: ", "mec").'</strong></span><span>'. $customer_email .'</span></div>';
            $payment_html .= '<div><span><strong>'.__("Customer Dates: ", "mec").'</strong></span><span>'. $mec_customer_date.'</span></div>';
            $payment_html .= '<div><span><strong>'.__("Transaction ID: ", "mec").'</strong></span><span>'. $transaction_id .'</span></div>';
            $inv_html = $payment_html.$inv_html;

            // Include the TCPDF Class
            if(!class_exists('TCPDF')) require_once MEC_ABSPATH.'app'.DS.'api'.DS.'TCPDF'.DS.'tcpdf.php';
            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);

            // set document information
            $pdf->SetCreator(PDF_CREATOR);
            $pdf->SetAuthor('Shabbat');
            $pdf->SetTitle('Shabbat Invoice');
            $pdf->SetSubject('Shabbat Invoicel');
            $pdf->SetKeywords('TCPDF, PDF, shabbat');

            // set default header data
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, "Booking Invoice", "ct.throove.com",array(0, 64, 255), array(0, 64, 128));
           ...
           // Print text using writeHTMLCell()
            $pdf->writeHTMLCell(0, 0, '', '', $inv_html, 0, 1, 0, true, '', true);
