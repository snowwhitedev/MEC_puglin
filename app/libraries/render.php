...
class MEC_render extends MEC_base
{
    ...
    public function skin($skin, $atts = array())
    {
        /*****Custom by Al****/
        $shabbat_found = false;
        foreach (explode(',', trim($atts['label'], ', ')) as $label) {
            $term = get_term_by('term_id', $label, 'mec_label');
            if (isset($term->name) && $term->name == SHABBAT) {
                $shabbat_found = true;
                break;
            }
        }

        ($shabbat_found) ?  $atts['shabbat'] = true : $atts['shabbat'] = false;
    ...
    /**
     * Renders and Returns event dates
     * @author Webnus <info@webnus.biz>
     * @param int $event_id
     * @param object $event
     * @param int $maximum
    ...
    public function dates($event_id, $event = NULL, $maximum = 6, $today = NULL)
    {
        ...
        if (isset($event->mec->repeat) and $event->mec->repeat == '0') {
            ...
            if (in_array($repeat_type, array('daily', 'weekly'))) {
                ...
                if (!$this->main->is_past($finish_date['date'], $start_date) and !in_array($start_date, $exceptional_days))
                    $dates[] = array(
