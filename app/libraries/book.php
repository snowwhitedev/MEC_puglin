...
class MEC_book extends MEC_base
{
     ...
     * @return array
     */
    public function get_price_details_mde($tickets, $event_id, $event_tickets, $variations = array(), $customer_date)
    {
        $total = 0;
        $details = array();
        $total_tickets_count = count($tickets);
        $booked_tickets = array();
        foreach ($tickets as $ticket) {
            $customer_dates = explode(',', $customer_date);
            $booked_price = "";
            foreach ($customer_dates as $date) {
                $t_price = (isset($event_tickets[$ticket['id']]) and isset($event_tickets[$ticket['id']]['price'])) ? $this->get_ticket_price($event_tickets[$ticket['id']], $date) : 0;
                $total = $total + $t_price;
                //$booked_price .= $date . ":" . $this->main->render_price($t_price) . "    ";
                $booked_price = $t_price;
            }
            $ticket['ticket_id']    = $ticket['id'];
            $ticket['ticket_name']  = $this->get_ticket_name($event_id, $ticket['id']);
            $ticket['booked_price'] = $booked_price;
        ...
        if (isset($this->settings['ticket_variations_status']) and $this->settings['ticket_variations_status'] and is_array($variations) and count($variations)) {
            ...
            foreach ($ticket_variations as $key => $ticket_variation) {
                if (!is_numeric($key))
                    continue;
                if (!isset($ticket_variation['title']) or (isset($ticket_variation['title']) and !trim($ticket_variation['title'])))
                    continue;
        ...
        if (isset($this->settings['taxes_fees_status']) and $this->settings['taxes_fees_status']) {
            ...
            foreach ($fees as $key => $fee) {
                ...
                if ($fee['type'] == 'percent')
                    $fee_amount += (($total + $total_variations_amount) * $fee['amount']) / 100;
                elseif ($fee['type'] == 'amount')
                    $fee_amount += ($total_tickets_count * $fee['amount']);
        ...
        return array('total' => ($total + $total_fee_amount + $total_variations_amount), 'details' => $details, 'tickets' => $booked_tickets);
    }
    ...
    public function add($values, $transaction_id, $ticket_ids)
    {
        ...
        update_post_meta($book_id, 'mec_customer_date', $transaction['customer_date']);
    ...
    /**
     * Returns the ticket's name of an event for a certain ticket
     * @author Al
     * @param int $event_id
     * @param string $ticket_id
     * @return string
     */
    public function get_ticket_name($event_id, $ticket_id)
    {
        $tickets = get_post_meta($event_id, 'mec_tickets', true);

        // No Ticket Found!
        if (!is_array($tickets) or (is_array($tickets) and !count($tickets))) return "Unknown";

        return $tickets[$ticket_id]['name'];
    }
}
